    ...
    for i in range(1, args.n_epoch + 1):
        ...
        for j in range(n_train_batches):
            ...
            if args.model in ['KGNN', 'ASGCN']:
                optimizer.zero_grad()
                train_x, train_xt, train_y, train_pw, train_adj, train_mask = get_batch_input(dataset=train_set,
                                                                                              bs=args.bs, args=args,
                                                                                              idx=j)
                train_x, train_xt, train_y, train_pw, train_adj, train_mask = torch.from_numpy(
                ...
                optimizer.zero_grad()
                train_x, train_xt, train_tag, train_y, train_pw = get_batch_input(dataset=train_set, bs=args.bs,
                                                                                  args=args,
                                                                                  idx=j)
...
def train_bert(args, path, is_save=False, is_bert=True):
    ...
    for i in range(1, args.n_epoch + 1):
        ...
        for j in range(n_train_batches):
            ...
            optimizer.zero_grad()
            ######bert######
            train_x, train_xt, train_y, train_pw, train_adj, train_mask = get_batch_input_bert(dataset=train_set,
                                                                                               bs=args.bs,
                                                                                               args=args, idx=j)
        ...
        for j in range(n_test_batches):
            if args.model in ['KGNN', 'ASGCN']:
                ...
                test_x, test_xt, test_tag, test_y, test_pw = torch.from_numpy(test_x), torch.from_numpy(
                    test_xt), torch.from_numpy(test_tag), \
